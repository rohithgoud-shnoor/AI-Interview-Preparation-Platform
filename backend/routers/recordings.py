    ...
    try:
        with open(file_path, "rb") as f:
            ...
            data = {
                ...
                "prompt": "Umm, uh, haaa, ah, like, you know, so, actually."
            }
...
def analyze_recording_transcript(
    ...
    try:
        ...
        # Helper to process and count filler words using regex
        def process_filler_words(transcript_text: str, ai_filler_words: dict) -> dict:
            ...
            counts = {}
            total_count = 0
            for word_label, pattern in patterns.items():
                matches = re.findall(pattern, transcript_text, re.IGNORECASE)
                match_count = len(matches)
                if match_count > 0:
                    counts[word_label] = match_count
                    total_count += match_count
            if total_count > 0:
                details = []
                for word, count in counts.items():
                    details.append({"word": word, "count": count})
                details.sort(key=lambda x: x["count"], reverse=True)
                words_str = ", ".join([f'"{d["word"]}"' for d in details])
                feedback = f"We detected filler words ({words_str}) in your response. Try to slow down or pause when searching for words to convey confidence and clarity."
                return {
        ...
        data["filler_words"] = process_filler_words(chunks_text, ai_filler)
